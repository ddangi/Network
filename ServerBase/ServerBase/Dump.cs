...
namespace ServerBase
{
    ...
    public class Dump
    {
        public static void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            string miniDumpName = string.Format($"miniDump_{strNow}.dmp");
            string fullDumpName = string.Format($"fullDump_{strNow}.dmp");
            string path = Environment.CurrentDirectory + "\\dump\\";
            if(false == Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string miniDumpFile = System.IO.Path.Combine(path, miniDumpName);
            string fullDumpFile = System.IO.Path.Combine(path, fullDumpName);
            MiniDumper.Write(miniDumpFile, MiniDumper.Typ.MiniDumpNormal); ;
            MiniDumper.Write(fullDumpFile);
        }
    }
